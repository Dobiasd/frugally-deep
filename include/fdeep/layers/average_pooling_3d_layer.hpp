...
void inner_average_pool_channels_first(const tensor& in, tensor& out,
    ...
    int pad_front_int, int pad_top_int, int pad_left_int)
{
    ...
    out.set_ignore_rank(tensor_pos(z, d4, y, x), val / static_cast<float_type>(divisor));
}

void inner_average_pool_channels_last(const tensor& in, tensor& out,
    std::size_t pool_size_d4, std::size_t pool_height, std::size_t pool_width,
    std::size_t strides_d4, std::size_t strides_y, std::size_t strides_x,
    std::size_t d4, std::size_t y, std::size_t x, std::size_t z,
    int pad_front_int, int pad_top_int, int pad_left_int)
...
public:
    explicit average_pooling_3d_layer(const std::string& name,
        ...
        channels_first ? &inner_average_pool_channels_first : &inner_average_pool_channels_last)
    {
    }
};
