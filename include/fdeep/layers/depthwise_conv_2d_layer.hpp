...
public:
    explicit depthwise_conv_2d_layer(
        ...
        : layer(name),
        filters_depthwise_(generate_im2col_filter_matrix(
...
protected:
    tensors apply_impl(const tensors& inputs) const override
    {
        ...
        const auto result = depthwise_convolve(strides_, padding_, filters_depthwise_, input);
        assertion(result.shape().depth_ == input.shape().depth_,
            "Invalid output shape");
        return {result};
    }

    convolution_filter_matrices filters_depthwise_;
    shape2 strides_;
    padding padding_;
};
