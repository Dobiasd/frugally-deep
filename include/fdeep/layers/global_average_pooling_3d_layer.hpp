...
{
public:
    explicit global_average_pooling_3d_layer(const std::string& name, bool channels_first) :
        ...
        for (std::size_t z = 0; z < feature_count; ++z)
        {
            ...
            {   
                for (std::size_t y = 0; y < in_height; ++y)
                {
                    ...
                    {
                        if (channels_first_)
                            val += in.get_ignore_rank(tensor_pos(z, d4, y, x));
                        else
                            val += in.get_ignore_rank(tensor_pos(d4, y, x, z));
                    }
                }
            }
            out.set_ignore_rank(tensor_pos(z), val / static_cast<float_type>(in_size_dim_4 * in_height * in_width));
        }
        return out;
    }
};
