...
namespace internal {
    ...
    protected:
        ...
        // input shape: (T, dim). kernel shape: (dim, num_heads, head_dim).
        // bias shape: (num_heads, head_dim). out shape: (T, num_heads, head_dim).
            ...
            std::size_t num_heads) const
        {
            ...
            const auto& bv = has_bias ? *bias->as_vector() : dummy_bias;
            ...
            tensor result(tensor_shape(T, num_heads, head_dim_), std::move(out));
            return result;
        }
        ...
        tensors apply_impl(const tensors& input) const override
        {
            ...
            tensor gate(tensor_shape(static_cast<std::size_t>(1)), float_type(0));
            if (use_gate_) {
                const tensor* gb = use_bias_ ? &gate_bias() : nullptr;
                gate = project(query_raw, gate_kernel(), gb, num_query_heads_);
            }
            ...
            // attn shape: (T_q, num_query_heads, head_dim)
            ...
            for (std::size_t h_q = 0; h_q < num_query_heads_; ++h_q) {
                ...
                for (std::size_t tq = 0; tq < T_q; ++tq) {
                    // Compute scores over keys for this (h_q, tq).
            ...
            // Apply gate: attn = sigmoid(gate) * attn (element-wise, same shape).
            if (use_gate_) {
                const auto& gv = *gate.as_vector();
                for (std::size_t i = 0; i < attn.size(); ++i) {
                    const float_type sig = static_cast<float_type>(1)
                        / (static_cast<float_type>(1) + std::exp(-gv[i]));
                    attn[i] *= sig;
                }
            }

            // Output projection: out[t, d] = sum_{h, c} attn[t, h, c] * o_kernel[h, c, d] + o_bias[d]
            const tensor& out_kernel = o_kernel();
            const std::size_t out_dim = out_kernel.shape().depth_;
            const auto& Ov = *out_kernel.as_vector();
            const float_vec dummy_obv;
            const auto& obv = use_bias_ ? *o_bias().as_vector() : dummy_obv;
